...
export default function SettingsPage() {
  ...
  const [showPermanentTokenGuide, setShowPermanentTokenGuide] = useState(false);
      ...
      if (res.ok) {
        ...
        if (json.success && json.data) {
          if (json.data.status === "AUTH_ERROR") {
            setShowPermanentTokenGuide(true);
          }
          setWabaConfig({
  ...
  const handleSaveWaba = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        if (json.isTokenExpired || json.message?.toLowerCase().includes("expired") || json.message?.includes("190")) {
          setShowPermanentTokenGuide(true);
        }
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => setShowPermanentTokenGuide((prev) => !prev)}
                        className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 hover:underline flex items-center gap-1 cursor-pointer"
                      >
                        <HelpCircle className="w-3.5 h-3.5" />
                        <span>How to get permanent token?</span>
                        {showPermanentTokenGuide ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                      </button>
                    </div>
                    ...
                    {wabaConfig.status === "AUTH_ERROR" && (
                      <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/60 text-xs space-y-1">
                        <div className="flex items-center gap-1.5 text-rose-800 dark:text-rose-300 font-bold">
                          ...
                          <span>Token Expired or Invalid (Meta Error 190)</span>
                        </div>
                        <p className="text-[11px] text-rose-700 dark:text-rose-300 leading-relaxed">
                          Your Meta access token has expired. Developer tokens from <em>WhatsApp &gt; API Setup</em> expire after 24 hours. Generate a <strong>Permanent System User Token</strong> below that never expires.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full pl-3.5 pr-10 py-2.5 bg-slate-50 dark:bg-slate-900 border rounded-xl text-xs font-mono font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 ${
                          wabaConfig.status === "AUTH_ERROR"
                      ...
                      <button
                        ...
                        className="absolute right-2.5 top-2.5 p-1 rounded-md text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer"
                    ...
                    {/* Step-by-Step Permanent Token Instructions Card */}
                    {showPermanentTokenGuide && (
                      <div className="p-4 rounded-xl bg-slate-100/90 dark:bg-slate-900/90 border border-emerald-500/40 text-xs space-y-3 shadow-2xs mt-2 animate-in fade-in duration-150">
                        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                          <span className="font-extrabold text-slate-900 dark:text-white flex items-center gap-1.5">
                            <Key className="w-4 h-4 text-emerald-600" />
                            <span>Quick 4-Step Permanent Token Setup (Never Expires)</span>
                          </span>
                          <a
                        ...
                        <div className="space-y-2.5 text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
                          <div className="flex items-start gap-2">
                            <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">1</span>
                            <div>
                              <strong className="text-slate-900 dark:text-white">Create/Select an Admin System User:</strong> Open <a href="https://business.facebook.com/settings/system-users" target="_blank" rel="noopener noreferrer" className="text-emerald-600 dark:text-emerald-400 underline font-medium">Meta Business Settings &gt; Users &gt; System Users</a>. Click <strong>Add</strong>, name it (e.g. <em>Broadcast Manager</em>), and choose role <strong>Admin</strong>.
                            </div>
                          </div>

                          <div className="flex items-start gap-2">
                            <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">2</span>
                            <div>
                              <strong className="text-slate-900 dark:text-white">Assign Assets:</strong> Click <strong>Assign Assets</strong> &gt; Select your <strong>App</strong> and <strong>WhatsApp Business Account</strong> &gt; Toggle on <strong>Full Control / Manage WhatsApp</strong> &gt; Save.
                            </div>
                          </div>

                          <div className="flex items-start gap-2">
                            <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">3</span>
                            <div>
                              <strong className="text-slate-900 dark:text-white">Generate Permanent Token:</strong> Click <strong>Generate New Token</strong>. Select your WhatsApp App, set <strong>Token Expiration</strong> to <strong>Never</strong>, and check these 2 permissions:
                              <div className="flex flex-wrap gap-1.5 mt-1">
                                <span className="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 font-mono text-[10px] font-bold border border-emerald-300/40">whatsapp_business_messaging</span>
                                <span className="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 font-mono text-[10px] font-bold border border-emerald-300/40">whatsapp_business_management</span>
                              </div>
                            </div>
                          </div>

                          <div className="flex items-start gap-2">
                            <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">4</span>
                            <div>
                              <strong className="text-slate-900 dark:text-white">Copy &amp; Paste:</strong> Copy the generated token string (<code className="text-[10px]">EAABwz...</code>), paste it into the field above, and click <strong>Save Credentials</strong>.
                            </div>
                          </div>
                        </div>
                      </div>
